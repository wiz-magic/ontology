...
### 9.5 Connector contract (data-source connectors)

A **connector** is any generator that continuously feeds an external source (chat threads, code repositories, ticket systems, team databases) into the memory layer. The contract is deliberately uniform — the memory-layer analog of "every source lands in one table": **every connector, regardless of source, emits the same staging file kinds** (§9.1 candidates/edges, §9.2 synthesis). A connector that honors this contract needs no special handling anywhere else in the system, and any team can add one.

Rules (in addition to the §9 firewall, which applies unchanged):

1. **Write scope** — a connector writes only `staging/candidates/`, `staging/edges/`, `staging/synthesis/`, and appends `[GEN]` lines to `STAGING-LOG.md` (§9.3). Nothing else, ever.
2. **Distill, don't dump** — never paste raw transcripts into candidates. Normalize each unit into the candidate skeleton: `title` = the one line a person would actually search for; `Proposed Facts` = one distilled statement per line (for a discussion thread: the question, the resolution, the systems/IDs mentioned). Retrieval and promotion quality both come from this normalization.
3. **Provenance** — `sources` must point at the real source (file path, thread URL, commit hash). The §9.1 provenance rule applies unchanged: the connector itself is never a source.
4. **Granularity** — one candidate per coherent unit (one thread, one document section, one row-group), not one per message. If a sub-unit carries standalone value that the unit-level summary would bury, emit it as its own candidate citing the same real source.
